...
1. **Startup**  
   ...
   **한국어:** 설정을 불러오고, `OPENAI_API_KEY`를 확인한 뒤, 실험 ID(예: `mof-20260212-1234`)를 생성합니다. 이전 실행 기록을 메모리에서 읽어 상위 k개를 **memory context** 문자열로 만듭니다.
...
2. **Intent** (1st LLM call)  
   ...
   **한국어:** 사용자 질의와 memory context가 LLM에 전달됩니다. LLM이 **QueryIntent**를 반환합니다: MoF 이름(예: UiO-66), 목표, 작업 유형(예: relaxation), 부족한 입력, 그리고 MLIP로 수행 가능한지 여부입니다.
...
3. **Canonicalize** (2nd LLM call)  
   ...
   **한국어:** 원래 질의와 intent(JSON), memory context가 LLM에 전달됩니다. 검색 및 스펙 생성에 맞는 하나의 정확한 문장 **CanonicalQuery**를 반환합니다(예: UiO-66에 대해 fmax 0.05 eV/Å, 최대 500 step으로 구조 완화를 수행하고 SevenNet용 JSON 스펙을 출력한다).  
...
4. **Retrieve** (no LLM)  
   ...
   **한국어:** **arXiv:** canonical query로 arXiv API를 호출해 최대 `arxiv_max_docs`개 결과를 가져와 **literature text**로 묶습니다. API 오류(500/429) 시 이 블록은 비우고 진행합니다. **Local PDFs:** `local_pdfs/` 안의 PDF를 PyMuPDF로 검색해, 상위 매칭 구간을 **local context**로 씁니다. 둘 다 길이 제한으로 잘라 프롬프트 크기를 맞춥니다.
...
5. **Novelty** (3rd LLM call)  
   ...
   **한국어:** canonical 질의, memory context, literature text(arXiv), local context(PDF)가 LLM에 전달됩니다. LLM이 **NoveltyVerdict**(pass / reject / uncertain)와 짧은 근거, 참고 문헌을 반환합니다. **reject**이면 판정을 출력하고 메모리에 기록한 뒤 스펙 없이 종료합니다. **pass** 또는 **uncertain**이면 다음 단계로 진행합니다.
...
6. **Spec** (4th LLM call)  
   ...
   **한국어:** 원래 질의, canonical 질의, memory context, novelty verdict(JSON), 실험 ID가 LLM에 전달됩니다. LLM이 **ExperimentSpec**(structure, calculator, task, postprocess, notes 등) 구조체를 반환합니다.
...
7. **Finish**  
   ...
   **한국어:** 스펙을 `outputs/<exp_id>.json`에 저장하고, 질의·판정·작업 유형 등을 메모리에 추가해 다음 실행에서 재사용할 수 있게 합니다.
...
**한국어 (빠른 모드):** 빠른 모드에서는 LLM 호출 3회(intent → canonicalize → spec), arXiv 호출 없음, 더 적은 메모리·더 짧은 context로 동작합니다. 실행이 빨라지고 토큰 사용량이 줄며, novelty 단계는 생략되고 모두 pass로 처리됩니다.
